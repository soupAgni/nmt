...
#modifications ---------			
learning_rate_pick = 10**np.random.uniform(-3, -1)
optimizer_pick = ["sgd", "adam"]
attention_pick = ["luong", "scaled_luong" , "bahdanau", "normed_bahdanau"]
attention_architecture_pick = ["standard", "gnmt", "gnmt_v2"]
infer_mode_pick = ["greedy", "sample", "beam_search"]
beam_width = np.random.randint(3, 6)
...
def create_hparams(flags):
  ...
  return tf.contrib.training.HParams(
      ...
      # Attention mechanisms
      attention= attention_pick[np.random.randint(0,4)], #flags.attention,
      attention_architecture= attention_architecture_pick[np.random.randint(0,3)], #flags.attention_architecture,
      output_attention=flags.output_attention,
      pass_hidden_state=flags.pass_hidden_state,

      # Train
      optimizer= optimizer_pick[np.random.randint(0,2)],#flags.optimizer,
      num_train_steps=flags.num_train_steps,
      batch_size=flags.batch_size,
      init_op=flags.init_op,
      init_weight=flags.init_weight,
      max_gradient_norm=flags.max_gradient_norm,
      learning_rate= learning_rate_pick, #flags.learning_rate,
      ...
      # Advanced inference arguments
      infer_mode= infer_mode_pick[np.random.randint(0,3)],#flags.infer_mode,
      beam_width=beam_width,#flags.beam_width,
  ...
  if flags.inference_input_file:
    ...
    if ref_file and tf.gfile.Exists(trans_file):
      for metric in hparams.metrics:
        ...
        utils.print_out("-----------------------------Stats -----------------------------------")
        utils.print_out("learning rate: %.lf" %hparams.learning_rate)
        utils.print_out("num training steps: %d" %hparams.num_train_steps)
        utils.print_out("optimizer: %s" %hparams.optimizer)
        utils.print_out("type of attention: %s" %hparams.attention)
        utils.print_out("attention architecture: %s" %hparams.attention_architecture)
        utils.print_out("inference mode: %s" %hparams.infer_mode)
        if (hparams.infer_mode == "beam_search"):
            utils.print_out("beam width %d" %hparams.beam_width)
